...
HOME_DIR="/home/user/Desktop/oss-arch-gym"
os.chdir(HOME_DIR)

# Define global variables
PERF_LOG_PATH = './project_ssd/profiling_tools/results/performance_log.csv'
CONFIG_PATH = Path("./project_ssd/tuning_tools/config.json")
LINUX_TUNE_CONFIG_PATH = './project_ssd/tune_config_wprof.sh'
POWERSHELL_LINUX_TUNE_CONFIG_PATH = r"C:\Users\Administrator\Desktop\wsl\arch-gym\main.ps1"
LOG_DIR = './project_ssd/log'
...
class CustomEnv(gym.Env):
    def __init__(self, max_steps=10):
        ...
        self.done = False
        self.ideal = np.array([50000.0], dtype=np.float32) #ideal values for throughput
...
def main():
    ...
    problem.search_space.select_root().add_categorical_param(name='PowerState', feasible_values=[0])
    ...
    if ALGORITHM == "bo":
      designer = EmukitDesigner(problem, num_random_samples=5)
    elif ALGORITHM == "random":
      designer = RandomDesigner(problem.search_space, seed = SEED)
    else:
    ...
    for i in range(NUM_STEPS):
      ...
      for suggestion in suggestions:
          ...
          print(f"Iteration {i} Observation: {obs}")
          
          # Update agent
          final_measurement = vz.Measurement({'Reward': reward})
          suggestion.to_trial().complete(final_measurement)
      trails = [suggestion.to_trial().complete(final_measurement)]
      designer.update(vza.CompletedTrials(trails))
    end_time = time.time()
