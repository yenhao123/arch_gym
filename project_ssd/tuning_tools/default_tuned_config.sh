...
function read_config_json() {
    config_path="$home_dir/default_config.json"
...
if [ "$weight" -ge 1 ] && [ "$weight" -le 6 ] && [ "$burst" -ge 0 ] && [ "$burst" -le 7 ]; then
    ...
    echo "Error: Invalid Arbitration Value!"
    exit 1
fi
...
if [ "$power_state" -ge 0 ] && [ "$power_state" -le 4 ]; then
    ...
    echo "Error: Invalid Power Management Value!"
    exit 1
fi
...
if [ "$ic_state" -ge 0 ] && [ "$ic_state" -le 5 ]; then
    ...
    echo "Error: Invalid Interrupt Coalescing Value!"
    exit 1
fi
...
if [ "$atomicity_state" -ge 0 ] && [ "$atomicity_state" -le 1 ]; then
    ...
    echo "Error: Invalid Atomicity Policy Value!"
    exit 1
fi
